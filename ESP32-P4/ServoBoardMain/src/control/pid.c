...
// 本文件只实现纯 PID 数学逻辑，不依赖 Arduino/FreeRTOS。
// 上层 ControlSolver 负责为每个关节保存 PID_Info_TypeDef 实例。
/**
...
{
        // 输出出现 NaN/INF 时锁定错误状态，下次计算会清空并返回 0。
		/* Judge NAN/INF */
...
{	
  // 位置式 PID 与增量式 PID 共用同一状态结构，由 Pid->type 决定计算路径。
