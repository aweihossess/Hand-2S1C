...
new_method = '''    def on_mcp_confirm_zero(self):
    ...
    # 删除旧的标定控制方法
    # _enable_mcp_calib_controls and _disable_mcp_calib_controls deleted
'''

# 查找旧方法的开始和结束位置
old_start_marker = '    def on_mcp_confirm_zero(self):'
old_end_marker = '    def _save_mcp_zero_config(self):'

start_idx = content.find(old_start_marker)
end_idx = content.find(old_end_marker)

if start_idx != -1 and end_idx != -1:
    print(f"找到旧方法: 从位置 {start_idx} 到 {end_idx}")
    # 替换内容
    new_content = content[:start_idx] + new_method + content[end_idx:]
    
    with open('main_window.py', 'w', encoding='utf-8') as f:
        f.write(new_content)
    print("✓ 文件已更新 - on_mcp_confirm_zero 方法已修复")
else:
