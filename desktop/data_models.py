...
class JointEncoder:
    ...
    """
    encoder_id: int = 0
    raw: int = 0              # 原始编码器计数（raw）
    mapped_count: int = 0     # 下位机已零位处理后的计数（mapped）
    ...
    def current_angle_deg(self) -> float:
        """当前角度（度）：优先使用下位机 mapped 计数，回退到本地 raw-zero_raw。"""
        if self.mapped_valid:
            return self.mapped_count * RAW_TO_DEG
...
class HandModel:
    ...
    has_servo_zero_offset_data: bool = False
    ...
    servo_software_zero_offsets: List[int] = field(default_factory=lambda: [0] * MOTOR_COUNT)
    servo_online: List[bool] = field(default_factory=lambda: [False] * MOTOR_COUNT)
    # 伺服单圈原始位置与在线状态（来自 PACKET_TYPE_SERVO_RAW，单位：单圈计数）
    ...
    joint_debug_target_length: List[float] = field(default_factory=lambda: [0.0] * ENCODER_COUNT)
    joint_debug_actual_length: List[float] = field(default_factory=lambda: [0.0] * ENCODER_COUNT)
    joint_debug_mapped_motor_target: List[float] = field(default_factory=lambda: [0.0] * ENCODER_COUNT)
    joint_debug_motor_zero_abs: List[int] = field(default_factory=lambda: [0] * ENCODER_COUNT)
    joint_debug_solver_output_pos: List[int] = field(default_factory=lambda: [0] * ENCODER_COUNT)
    joint_debug_zero_homing: List[bool] = field(default_factory=lambda: [False] * ENCODER_COUNT)
    ...
    joint_debug_timestamp_ms: List[int] = field(default_factory=lambda: [0] * ENCODER_COUNT)
    ...
    def copy_state_from(self, other: "HandModel") -> None:
        ...
        for i, e in enumerate(self.encoders):
            if i < len(other.encoders):
                ...
                e.mapped_count = other.encoders[i].mapped_count
                e.mapped_valid = other.encoders[i].mapped_valid
        ...
        self.servo_software_zero_offsets = list(other.servo_software_zero_offsets)
        ...
        self.has_servo_zero_offset_data = bool(other.has_servo_zero_offset_data)
        ...
        self.joint_debug_target_length = list(other.joint_debug_target_length)
        self.joint_debug_actual_length = list(other.joint_debug_actual_length)
        self.joint_debug_mapped_motor_target = list(other.joint_debug_mapped_motor_target)
        self.joint_debug_motor_zero_abs = list(other.joint_debug_motor_zero_abs)
        self.joint_debug_solver_output_pos = list(other.joint_debug_solver_output_pos)
        self.joint_debug_zero_homing = list(other.joint_debug_zero_homing)
        ...
        self.joint_debug_timestamp_ms = list(other.joint_debug_timestamp_ms)
        # 复制数据有效性标志
